...
void Visualization::applySlicing(std::vector<float> &scalarValues)
{
    ...
    for (int  i = m_DIM-1; i > 0; i--){
        my_CubeData[i] = my_CubeData[i-1];
    }

    my_CubeData[0] = scalarValues;



    std::vector<float> tmp = std::vector<float>(m_DIM * m_DIM, 0.0F);
    ...
    case SlicingDirection::x:
        ...
        for (int t = 0; t < (int)m_DIM; t++){
            for (int y = 0; y < (int)m_DIM; y++ ){
                tmp[m_DIM * t + y] = my_CubeData[t][m_DIM * m_sliceIdx + y];
            }
        }
    ...
    case SlicingDirection::y:
        // yIdx is constant
        for (int t = 0; t < (int)m_DIM; t++){
            for (int x = 0; x < (int)m_DIM; x++ ){
                tmp[m_DIM * t + x] = my_CubeData[t][m_DIM * x + m_sliceIdx];
            }
        }
        break;
