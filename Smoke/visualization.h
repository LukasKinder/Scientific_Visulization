...
class Visualization : public QOpenGLWidget, protected QOpenGLFunctions_3_3_Core
{
    ...
    bool m_drawScalarData = true;   // Draw the smoke or not.
                 // Size of simulation grid. Must be even.
    ...
    // Add code here
    std::vector<float> my_emptyVec = std::vector<float>(m_DIM * m_DIM, 0.0F);
    std::vector<std::vector<float>> my_CubeData = std::vector<std::vector<float>>(m_DIM, my_emptyVec);
